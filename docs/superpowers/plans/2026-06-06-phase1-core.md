...
1. **`computeStats` store access** (Task 4 note): confirm the public accessor on `@allurereport/core@3.9.0`; fall back to reading `widgets/summary.json` from `outputDir` if no public API exists. This is the single most likely API-shape surprise.
2. **`resolveConfig` plugin resolution**: verify the `{ plugins: { awesome: {...} } }` shorthand resolves `@allurereport/plugin-awesome` when invoked programmatically (it does via the CLI; confirm outside a CLI cwd). If not, import `AwesomePlugin` and construct a `PluginInstance` explicitly.
3. **ESM/`better-sqlite3` native build** under Node 20 in the slim Docker image — may need `python3`/`make`/`g++` in the build stage; add `apt-get install -y python3 build-essential` if `pnpm install` fails.
