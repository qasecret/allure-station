      ...
      {points.map((p, i) => {
        ...
        return (
          <g key={p.runId}>
            ...
            {/* Orange cap marks runs with flaky tests (the flakiness trend). */}
            {flaky > 0 && <rect x={i * 14} y={42 - h - 3} width={10} height={3} fill="#f0ad4e" pointerEvents="none" />}
          </g>
        );
      })}
      {anyFlaky && <text x={0} y={10} fontSize={9} fill="#f0ad4e">▮ flaky</text>}
    </svg>
  );
}
